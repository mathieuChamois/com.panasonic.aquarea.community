...
// `direction` dit seulement OU va l'energie : rien (0), le circuit de
// chauffage/rafraichissement (1), ou le ballon ECS (2). Le sens (chaud/froid)
// vient du mode etendu, cf. EXT_MODE_COOLING / EXT_MODE_HEATING.
const DIRECTIONS = { 0: 'idle', 1: 'active', 2: 'hot_water' };

// ExtendedOperationMode : 3 = auto en train de chauffer, 4 = auto en train de
// rafraichir. Le mode etendu dit donc toujours ce que la machine fait
// reellement, y compris en auto — contrairement au mode demande.
...
// 'heat', 'heat_tank' et 'dhw' partagent le meme mode climat (chauffage) ;
// 'cool' et 'cool_tank' partagent le meme mode climat (rafraichissement) ;
// seule l'autorisation ECS (tankStatus.operationStatus) les distingue, cf. setMode().
...
class AquareaClient {
  ...
  /**
   * Ecriture directe sur `remote/v1/api/devices/{guid}`, forme `status[]`.
   *
   * Certaines commandes (forceDHW, forceHeater, specialStatus, arret general)
   * ne passent pas par le proxy "transfer" mais par cet endpoint, avec le
   * champ `deviceGuid` repete dans l'element de statut. Forme alignee sur
   * aioaquarea (post_device_force_dhw / _force_heater / _set_special_status).
   *
   * @param {string} deviceId
   * @param {object} patch  Champs de statut a ecrire (hors deviceGuid).
   */
  ...
  /**
   * Liste les appareils rattaches au compte Comfort Cloud.
   * Le type 2 est une PAC Aquarea. Les autres types sont exposes a titre
   * experimental afin de pouvoir collecter leur structure via les diagnostics.
  ...
  async getDeviceData(deviceId, { diagnostic = false, deviceType = '2' } = {}) {
    ...
    // "Chauffage seul" vs "Chauffage + ECS", ou "Froid seul" vs "Froid + ECS" :
    // on regarde l'autorisation ECS.
    ...
    // Sens de fonctionnement reel. null = indetermine (machine a l'arret) :
    // l'appelant conserve alors la derniere valeur connue plutot que de
    // basculer arbitrairement sur "chauffage".
    let isCooling = null;
    if (EXT_MODE_COOLING.includes(opMode)) isCooling = true;
    else if (EXT_MODE_HEATING.includes(opMode)) isCooling = false;

    // Ce que fait la machine, avec le sens quand on peut le determiner :
    // 'active' (chaud ou froid, sens inconnu) devient 'heating' / 'cooling'.
    ...
    return {
      ...
      thermostatMode,
      // true = froid, false = chaud, null = indetermine. Vaut aussi en 'auto'.
      isCooling,
      ...
      // Ce que la machine est en train de faire :
      // repos / chauffage / rafraichissement / eau chaude.
      direction,
   ...
   *   - 'dhw'       : eau chaude seule (zone coupee, ballon ECS autorise).
   *   - 'cool'      : rafraichissement seul (ballon ECS coupe).
   *   - 'cool_tank' : rafraichissement + eau chaude (ballon ECS autorise).
   *   - 'auto'      : n'affecte pas l'etat du ballon (preserve).
   */
  async setMode(deviceId, mode) {
    ...
    //  - 'heat_tank' / 'cool_tank' => ballon autorise (operationStatus = 1)
    //  - 'dhw'                     => ballon autorise et zone coupee
    //  - 'heat' / 'cool'           => ballon coupe    (operationStatus = 0)
    //  - 'auto'                    => on ne touche pas au ballon (etat conserve)
    if (current.hasTank) {
  ...
  /** Definit le niveau du mode silencieux. */
  ...
  /** Active le mode puissant pour la duree demandee, ou le desactive. */
  ...
  /**
   * Definit la consigne de rafraichissement d'une zone (coolSet).
   *
   * Meme unite que heatSet pour la zone : temperature d'eau absolue, ou
   * decalage de loi d'eau si la zone est pilotee ainsi.
   * @param {string} deviceId
   * @param {number} value
   * @param {number} [zoneId=1]
   */
  ...
  /**
   * Marche/arret general de l'unite, sans toucher au mode de fonctionnement.
   * Contrairement a setMode('off') / setMode('heat'), l'allumage ne reinitialise
   * ni la zone ni l'autorisation ECS : la machine repart sur son dernier mode.
   * @param {string} deviceId
   * @param {boolean} on
   */
  ...
  /**
   * Force un cycle d'eau chaude sanitaire (bouton "eau chaude forcee").
   *
   * ⚠️  La PAC coupe d'elle-meme le forcage une fois la consigne de ballon
   *     atteinte : c'est une impulsion, pas un interrupteur permanent.
   * @param {string} deviceId
   * @param {boolean} on
   */
  ...
  /**
   * Force l'appoint electrique (resistance de secours).
   *
   * ⚠️  Consommation electrique directe, sans COP : a n'utiliser que
   *     ponctuellement (grand froid, montee en temperature anti-legionelle).
   * @param {string} deviceId
   * @param {boolean} on
   */
  ...
  /**
   * Applique le prereglage Eco / Confort / Normal.
   *
   * Les decalages appliques (ecoHeat, comfortHeat, ...) sont ceux configures
   * dans la telecommande ; l'API ne fait que selectionner lequel est actif.
   * @param {string} deviceId
